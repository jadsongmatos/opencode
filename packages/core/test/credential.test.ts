...
describe("Credential", () => {
  it.live("imports supported legacy auth.json credentials once", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          // Unset OPENCODE_DATABASE_URL so isPostgres() returns false for SQLite test
          const originalDbUrl = process.env.OPENCODE_DATABASE_URL
          ...
          try {
            ...
            if (originalDbUrl) process.env.OPENCODE_DATABASE_URL = originalDbUrl
          }
        }),
      ),
    ),
