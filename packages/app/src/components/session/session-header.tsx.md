# External tests for session-header.tsx

**Arquivo:** `packages/app/src/components/session/session-header.tsx`

## Checklist

- [ ] @opencode-ai/ui/app-icon
- [ ] @opencode-ai/ui/button
- [ ] @opencode-ai/ui/dropdown-menu
- [ ] @opencode-ai/ui/icon
- [ ] @opencode-ai/ui/icon-button
- [ ] @opencode-ai/ui/keybind
- [ ] @opencode-ai/ui/spinner
- [ ] @/utils/toast
...
**Consultas usadas no Horsebox:** `createEffect`, `solid-js createEffect`, `createMemo`, `solid-js createMemo`, `createSignal`, `solid-js createSignal`, `For`, `solid-js For`, `onMount`, `solid-js onMount`, `Show`, `solid-js Show`
...
**Consultas usadas no Horsebox:** `Portal`, `solid-js/web Portal`, `web Portal`

**Arquivos de teste encontrados:** 4
...
**Consultas usadas no Horsebox:** `useLayout`, `@/context/layout useLayout`, `/context/layout useLayout`, `layout useLayout`
...
**Consultas usadas no Horsebox:** `useServer`, `@/context/server useServer`, `/context/server useServer`, `server useServer`
...
**Consultas usadas no Horsebox:** `useSettings`, `@/context/settings useSettings`, `/context/settings useSettings`, `settings useSettings`
...
```ts
test('#746 - _.template settings are not modified.', function(assert) {
    assert.expect(1);
    var settings = {};
...
```ts
test('passing data', function(assert) {
    assert.expect(3);
...
```ts
test('Backbone.ajax', function(assert) {
    assert.expect(1);
...
#### Use Backbone.emulateHTTP as default.

```ts
test('Use Backbone.emulateHTTP as default.', function(assert) {
    assert.expect(2);
    var model = new Backbone.Model;
    model.url = '/test';
...
#### Use Backbone.emulateJSON as default.

```ts
test('Use Backbone.emulateJSON as default.', function(assert) {
    assert.expect(2);
    var model = new Backbone.Model;
    model.url = '/test';
...
#### #2928 - Pass along `textStatus` and `errorThrown`.

```ts
test('#2928 - Pass along `textStatus` and `errorThrown`.', function(assert) {
    assert.expect(2);
    var model = new Backbone.Model;
    model.url = '/test';
...
#### #2606 - Collection#create, success arguments

```ts
test('#2606 - Collection#create, success arguments', function(assert) {
    assert.expect(1);
    var collection = new Backbone.Collection;
    collection.url = 'test';
    collection.create({}, {
...
#### should use `_.templateSettings.imports._.templateSettings`

```ts
test('should use `_.templateSettings.imports._.templateSettings`', function(assert) {
      assert.expect(1);
...
#### should fallback to `_.templateSettings`

```ts
test('should fallback to `_.templateSettings`', function(assert) {
      assert.expect(1);
...
#### should not modify `_.templateSettings` when `options` are given

```ts
test('should not modify `_.templateSettings` when `options` are given', function(assert) {
      assert.expect(2);
...
**Consultas usadas no Horsebox:** `useSync`, `@/context/sync useSync`, `/context/sync useSync`, `sync useSync`

**Arquivos de teste encontrados:** 10

### ../../.sbomtest/repos/901466a5bb-lodash/vendor/backbone/test/sync.js

#### #1052 - `options` is optional.

```ts
test('#1052 - `options` is optional.', function(assert) {
    assert.expect(0);
    var model = new Backbone.Model();
    model.url = '/test';
    Backbone.sync('create', model);
...
#### #3470 - save and fetch with parse false

```ts
test('#3470 - save and fetch with parse false', function(assert) {
    assert.expect(2);
    var i = 0;
    var model = new Backbone.Model();
...
#### save with PATCH and different attrs

```ts
test('save with PATCH and different attrs', function(assert) {
...
#### save in positional style

```ts
test('save in positional style', function(assert) {
    assert.expect(1);
    var model = new Backbone.Model();
    model.sync = function(method, m, options) {
      options.success();
...
#### save with non-object success response

```ts
test('save with non-object success response', function(assert) {
    assert.expect(2);
    var model = new Backbone.Model();
    model.sync = function(method, m, options) {
...
#### save will pass extra options to success callback

```ts
test('save will pass extra options to success callback', function(assert) {
    assert.expect(1);
...
#### fetch will pass extra options to success callback

```ts
test('fetch will pass extra options to success callback', function(assert) {
    assert.expect(1);
    var SpecialSyncModel = Backbone.Model.extend({
      sync: function(method, m, options) {
        _.extend(options, {specialSync: true});
...
#### non-persisted destroy

```ts
test('non-persisted destroy', function(assert) {
    assert.expect(1);
...
#### `save` with `wait` sends correct attributes

```ts
test('`save` with `wait` sends correct attributes', function(assert) {
    assert.expect(5);
    var changed = 0;
...
#### save turns on parse flag

```ts
test('save turns on parse flag', function(assert) {
    var Model = Backbone.Model.extend({
...
**Consultas usadas no Horsebox:** `global`, `@/utils/persist global`, `/utils/persist global`, `persist global`, `Persist`, `@/utils/persist Persist`, `/utils/persist Persist`, `persist Persist`, `persisted`, `@/utils/persist persisted`, `/utils/persist persisted`, `persist persisted`

**Arquivos de teste encontrados:** 42
...
#### should persist placeholders

```ts
test('should persist placeholders', function(assert) {
      assert.expect(1);
...
```ts
test('should compare regexes', function(assert) {
      assert.expect(5);
