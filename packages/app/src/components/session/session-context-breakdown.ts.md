# External tests for session-context-breakdown.ts

**Arquivo:** `packages/app/src/components/session/session-context-breakdown.ts`
...
**Consultas usadas no Horsebox:** `Message`, `@opencode-ai/sdk/v2/client Message`, `opencode-ai/sdk/v2/client Message`, `client Message`, `Part`, `@opencode-ai/sdk/v2/client Part`, `opencode-ai/sdk/v2/client Part`, `client Part`

**Arquivos de teste encontrados:** 94
...
#### `_.partition` should use `_.iteratee` internally

```ts
test('`_.partition` should use `_.iteratee` internally', function(assert) {
      assert.expect(1);
...
#### `_.' + methodName + '` creates a function that can be invoked with additional arguments

```ts
test('`_.' + methodName + '` creates a function that can be invoked with additional arguments', function(assert) {
      assert.expect(1);
...
#### `_.' + methodName + '` works when there are no partially applied arguments and the created function is invoked without additional arguments

```ts
test('`_.' + methodName + '` works when there are no partially applied arguments and the created function is invoked without additional arguments', function(assert) {
      assert.expect(1);
...
#### `_.' + methodName + '` should support placeholders

```ts
test('`_.' + methodName + '` should support placeholders', function(assert) {
      assert.expect(4);
...
#### `_.' + methodName + '` should use `_.placeholder` when set

```ts
test('`_.' + methodName + '` should use `_.placeholder` when set', function(assert) {
      assert.expect(1);
...
#### should work with combinations of partial functions

```ts
test('should work with combinations of partial functions', function(assert) {
      assert.expect(1);
...
#### should ensure `new combo` is an instance of `func`

```ts
test('should ensure `new combo` is an instance of `func`', function(assert) {
      assert.expect(2);
