# External tests for session-new-view.tsx

**Arquivo:** `packages/app/src/components/session/session-new-view.tsx`
...
**Consultas usadas no Horsebox:** `Show`, `solid-js Show`, `createMemo`, `solid-js createMemo`
...
**Consultas usadas no Horsebox:** `Mark`, `@opencode-ai/ui/logo Mark`, `opencode-ai/ui/logo Mark`, `logo Mark`
...
```ts
test('should deburr combining diacritical marks', function(assert) {
      assert.expect(1);
...
```ts
test('should work with compound words containing diacritical marks', function(assert) {
      assert.expect(3);
...
```ts
test('should not treat punctuation as words', function(assert) {
      assert.expect(1);
