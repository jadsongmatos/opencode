  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
        ...
        yield* Effect.promise(() => Storage.write(["session_diff", input.sessionID], diffs))
      ...
      const diff = Effect.fn("SessionSummary.diff")(function* (input: {
        ...
        const diffs = yield* Effect.promise(() =>
          Storage.read<Snapshot.FileDiff[]>(["session_diff", input.sessionID]).catch(() => []),
        )
        ...
        if (changed) void Storage.write(["session_diff", input.sessionID], next).catch(() => {})
        return next
      })
